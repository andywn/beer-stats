...
col2 <- brewer.pal(n=9, name="Oranges")
...
abline(lm(beers[beers$ibu > 0, ]$score~beers[beers$ibu > 0, ]$ibu), col=col2[6])
```
Summary
```{r summary ibu}
...
abline(lm(beers[beers$abv > 0, ]$score~beers[beers$abv > 0, ]$ibu), col=col2[6])
...
par(mai=c(1,2,1,1))
barplot(types, horiz=TRUE,las=1, col=c(col1[6], col2[6]), main="Average Rating by Subtype", xlab="Mean Rating")
```
IPA Subtypes
```{r ipa subtypes}
complist <- beers[beers$summary.type == 'IPA',]
summary.agglist <- aggregate(complist$score, by=list(complist$type), FUN=mean)
types.order <- order(as.numeric(summary.agglist$x),decreasing = TRUE)
types <- summary.agglist[types.order,]$x
names(types) <- summary.agglist[types.order,]$Group.1
par(mai=c(1,3,1,0))
barplot(types, horiz=TRUE,las=1, col=c(col1[6], col2[6]), main="Average Rating of IPA Types", xlab="Mean Rating")
```
Histograms of Checkins
```{r histogram}
# histogram of checkins
hist(beers$score, breaks = seq(-0.25,5.25,by=0.5), col=c(col1[6], col2[6]), main="Histogram of Ratings, Rounded to Nearest 0.5", xlab="Rating")
```

```{r histogrampostquarter}
# histogram after quarter points
hist(beers[beers$datetime > as.POSIXct("2015-07-09"), ]$score, breaks = seq(0.125,5.125,by=0.25), col=c(col1[6], col2[6]), main="Histogram of Ratings Post August 2015, Unrounded", xlab="Rating")
```
Moving Averages
```{r moving average}
...
```{r time}
# time of day
hist(beers$hour, breaks = 24, col=c(col1[6], col2[6]), main="Beers Drunk Based on Checkin Time", xlab="Hour")
```
```{r time rating}
...
barplot(height=agglist_hour[order(agglist_hour$Group.1),]$x, names.arg=agglist_hour[order(agglist_hour$Group.1),]$Group.1, xlab="Hour of the Day", ylab="Average Rating", col=c(col1[6], col2[6]), font=2)
